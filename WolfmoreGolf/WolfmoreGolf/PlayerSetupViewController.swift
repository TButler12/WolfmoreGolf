...
final class PlayerSetupViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction private func resetGameTapped(_ sender: UIButton) {
        // iPhone: action sheet; iPad: alert (no popover hassle)
        ...
        // Anchor action sheet to the button (needed if style == .actionSheet on iPhone Plus/Max)
    ...
    @IBAction private func debugGoToGameTap(_ sender: UIButton) {
        print("DEBUG: debugGoToGameTap fired")
    }
    ...
    @IBAction private func goToGameTapped(_ sender: UIButton) {
        ...
        view.endEditing(true)
        print("goToGameTapped tapped")   // debug: should see this every time
        ...
        let hasActiveNamed: Bool = (0..<min(nameFields.count, activateButtons.count)).contains { i in
            ...
            let name = (nameFields[safe: i]?.text ?? "")
        ...
        // 🚀 actually navigate to the game screen
    ...
    private func styleActivate(_ button: UIButton) {
        ...
        if #available(iOS 15.0, *) {
            ...
            if !button.isEnabled {
                ...
                button.setTitle("Max 9", for: .normal)
            } else {
