...
private let BILTMORE_PARS: [Int] = [
    4,4,4,4,3,5,3,4,4,  4,4,3,4,4,5,3,4,5
]
private let BILTMORE_HCS: [Int] = [
    4,8,14,10,16,2,18,6,12,  11,3,15,1,13,7,17,9,5
]
...
// ===================================================
// MARK: - CourseSetupViewController (your screen)
// ===================================================
...
final class CourseSetupViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItems = [
            UIBarButtonItem(title: "Load",   style: .plain, target: self, action: #selector(loadButtonTapped(_:))),
            UIBarButtonItem(title: "Manage", style: .plain, target: self, action: #selector(manageTapped(_:)))
        ]
        ...
        if let g = GameManager.shared.currentGame,
           ...
           g.courseHCToPass.count == 18 {
            ...
            applyToUIAndModel(name: "Current Course",
                              pars: g.courseParToPass,
                              hcs:  g.courseHCToPass)

        } else {
            applyToUIAndModel(name: "Biltmore CC",
                              pars: BILTMORE_PARS,
                              hcs:  BILTMORE_HCS)
        }
    }
    ...
    @objc private func manageTapped(_ sender: Any) {
        ...
        let ac = UIAlertController(title: "Manage \(titleName)", message: nil, preferredStyle: .actionSheet)

        // Save Edits → overwrite loaded course (or Save As… if none)
        ac.addAction(UIAlertAction(title: "Save Edits", style: .default) { [weak self] _ in
        ...
        ac.addAction(UIAlertAction(title: "Set to Biltmore Defaults", style: .default) { [weak self] _ in
            ...
            if let b = CourseLibrary.shared.biltmore() {
                self.applyToUIAndModel(name: b.name, pars: b.pars, hcs: b.hcs)
        ...
        if let id = activeCourseID, let current = CourseLibrary.shared.get(id: id),
    ...
    private func saveEditsOverActiveOrSaveAs() {
        ...
        if let id = activeCourseID, let existing = CourseLibrary.shared.get(id: id) {
            let updated = CourseProfile(id: existing.id, name: existing.name, pars: pars, hcs: hcs)
            CourseLibrary.shared.upsert(updated)
            applyToUIAndModel(name: updated.name, pars: updated.pars, hcs: updated.hcs)
    ...
    private func resetUnsavedToActive() {
        guard let id = activeCourseID, let c = CourseLibrary.shared.get(id: id) else { return }
        applyToUIAndModel(name: c.name, pars: c.pars, hcs: c.hcs)
        UINotificationFeedbackGenerator().notificationOccurred(.success)
    }

    // Confirm + delete the active course; then fall back to Biltmore
    private func confirmDeleteActiveCourse() {
        guard let id = activeCourseID, let current = CourseLibrary.shared.get(id: id) else { return }
    ...
    // Track which course is currently loaded from the library
    private var activeCourseID: UUID?

    /// Replace your old loader: show a course picker (Biltmore + saved)
    @IBAction private func loadButtonTapped(_ sender: Any) {
        ...
        guard !courses.isEmpty else {
            applyToUIAndModel(name: "Biltmore CC", pars: BILTMORE_PARS, hcs: BILTMORE_HCS)
            activeCourseID = nil
            NotificationCenter.default.post(name: .reloadUI, object: nil)
            return
        }
        ...
        for course in courses {
            ac.addAction(UIAlertAction(title: course.name, style: .default) { [weak self] _ in
                guard let self = self else { return }
                self.applyToUIAndModel(name: course.name, pars: course.pars, hcs: course.hcs)
    ...
    // Optional: hook this to a separate “Save As…” button if you have one
    
    @IBAction private func editButtonTapped(_ sender: UIButton) {
        manageTapped(sender)          // shows Save Edits / Reset / Set to Biltmore / Delete
    }
    ...
    private func applyToUIAndModel(name: String, pars: [Int], hcs: [Int]) {
        // Paint fields
        for (i, field) in parFields.sorted(by: { $0.tag < $1.tag }).enumerated() where i < 18 {
            field.text = "\(pars[i])"
        }
        for (i, field) in hcFields.sorted(by: { $0.tag < $1.tag }).enumerated() where i < 18 {
            field.text = "\(hcs[i])"
        }
    ...
    private func promptSaveCurrentAsNew() {
        ...
        nameAC.addAction(UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            guard let self = self else { return }

            // 1. Get entered name (or fallback)
            let rawName = nameAC.textFields?.first?.text?
                .trimmingCharacters(in: .whitespacesAndNewlines)
            let finalName = (rawName?.isEmpty == false) ? rawName! : "Unnamed Course"

            // 2. Build a new profile with a fresh UUID
            let newCourse = CourseProfile(name: finalName, pars: pars, hcs: hcs)

            // 3. Persist it
            CourseLibrary.shared.upsert(newCourse)

            // 4. Mark it as the active course so "Save Edits" / "Delete" know which one
            self.activeCourseID = newCourse.id

            // 5. Push values into UI and GameManager
            self.applyToUIAndModel(name: newCourse.name, pars: newCourse.pars, hcs: newCourse.hcs)

            // 6. Notify rest of app (score screen, etc.)
            NotificationCenter.default.post(name: .reloadUI, object: nil)

            // (Optional: nice haptic)
            UINotificationFeedbackGenerator().notificationOccurred(.success)
        })
    ...
    // ===================================================
    // MARK: - Your existing save/load (kept, just tidied)
    // ===================================================
...
extension CourseLibrary {
    ...
    func delete(id: UUID) { courses.removeAll { $0.id == id }; // save
        let u = UserDefaults.standard
        let data = (try? JSONEncoder().encode(courses)) ?? Data()
        u.set(data, forKey: "course.library.v1")
    }
    func biltmore() -> CourseProfile? {
        courses.first { $0.name.caseInsensitiveCompare("Biltmore CC") == .orderedSame }
    }
}
