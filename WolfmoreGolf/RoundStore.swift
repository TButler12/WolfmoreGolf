...
extension RoundStore {
    ...
    /// Returns the newest saved game's rows (one row per player), based on date.
    func newestGameRows() -> [RoundSummary]? {
        guard !rounds.isEmpty else { return nil }

        let grouped = Dictionary(grouping: rounds, by: \.gameID)
        let newest = grouped
            .map { (gameID, rows) -> (UUID, Date) in
                (gameID, rows.map(\.date).max() ?? .distantPast)
            }
            .max(by: { $0.1 < $1.1 })?.0

        guard let newestGameID = newest else { return nil }
        return grouped[newestGameID]
    }

    /// If you have the gameID you just recorded, use this (best).
    func rows(for gameID: UUID) -> [RoundSummary] {
        rounds.filter { $0.gameID == gameID }
    }
}
