...
final class GameViewController: UIViewController {
   ...
   // @IBOutlet weak var hammerLabel: UILabel!
    ...
    @IBAction private func endRoundTapped(_ sender: UIButton) {
        // 1) Must have a current game
        guard GameManager.shared.currentGame != nil else {
            let a = UIAlertController(
                title: "No Current Round",
                message: "Start or load a round before saving to history.",
                preferredStyle: .alert
            )
        ...
        // 2) Must have an owner name saved
        guard let rawName = ProfileStore.name else {
            let a = UIAlertController(
        ...
        ac.addAction(UIAlertAction(title: "Yes, Save", style: .default) { [weak self] _ in
            ...
            for friend in trackedFriends {
                // Only saves if that friend's name is on an ACTIVE seat
                if RoundStore.shared.recordFromCurrentGame(playerNameOverride: friend.name) != nil {
            ...
            var msg = "Added round for \(meSummary.playerName)."
            if !savedFriendNames.isEmpty {
                msg += "\nAlso saved for: " + savedFriendNames.joined(separator: ", ")
            }
        ...
        if let pop = ac.popoverPresentationController {
            ...
            pop.sourceRect = (sender as? UIView)?.bounds ?? .zero
        }
        present(ac, animated: true)
    }
